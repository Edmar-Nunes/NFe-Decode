...
function exportToPDF() {
    ...
    try {
        ...
        // Criar documento em modo paisagem (A4 paisagem = 297mm x 210mm)
        const doc = new jsPDF({
        ...
        const margin = 10; // Margens pequenas para aproveitar máximo espaço
        let yPos = margin;
        ...
        // Seção DESTINATÁRIO - lado direito
        const rightColumn = pageWidth / 2;
        yPos = margin + 12; // Reset yPos para alinhar com emitente
        doc.setFontSize(11);
        doc.setFont("helvetica", "bold");
        ...
        yPos = margin + 45; // Posição para a tabela
        ...
        const tableHeaders = [
            ["Item", "Código", "Descrição", "Qtd", "Unid.", "Valor Unit.", "Valor Total"]
        ];
        
        const tableData = produtosExibidos.map((prod, index) => {
            // Descrição reduzida se for muito longa
        ...
        const totalGeral = produtosExibidos.reduce((sum, prod) => {
            return sum + parseFloat(prod.valorTotal || 0);
        }, 0);
        ...
        // Configurar larguras das colunas para usar toda a largura da página
        const availableWidth = pageWidth - (2 * margin);
        const columnStyles = {
            0: { cellWidth: 15, halign: 'center' },    // Item
            1: { cellWidth: 25, halign: 'left' },      // Código
            2: { cellWidth: 100, halign: 'left' },     // Descrição (mais larga)
            3: { cellWidth: 20, halign: 'right' },     // Qtd
            4: { cellWidth: 20, halign: 'center' },    // Unid.
        ...
        doc.autoTable({
            ...
            didDrawCell: function(data) {
                ...
                // Destacar célula do valor total
            ...
            willDrawPage: function(data) {
                // Adicionar borda da página (opcional)
                doc.setDrawColor(0, 0, 0);
                doc.setLineWidth(0.5);
                doc.rect(margin - 5, margin - 5, 
        ...
        // Se houver informações adicionais, adicionar em nova página se necessário
        if (nfeData.informacoesAdicionais && nfeData.informacoesAdicionais !== "Nenhuma informação adicional") {
